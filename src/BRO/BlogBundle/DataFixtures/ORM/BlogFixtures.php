...
namespace BRO\BlogBundle\DataFixtures\ORM;
...
class BlogFixtures extends AbstractFixture implements OrderedFixtureInterface {
    ...
    public function load(ObjectManager $manager) {
        ...
        $blog6 = new Blog();
        $blog6->setTitle('Привет лунатикам');
        $blog6->setBlog('А в это время....');
        $blog6->setImage('one_or_zero12.jpg');
        $blog6->setAuthor('Бразер');
        $blog6->setTags('binary, one, zero, alive, dead, !trusting, movie, космос');
        ...
        for ($i = 1; $i <= 20; $i++) {
            ...
            $blog->setBlog('Text here '. $i);
            $blog->setImage("automatic_$i.jpg");
            $blog->setAuthor('Бразер');
            $blog->setTags('php');
